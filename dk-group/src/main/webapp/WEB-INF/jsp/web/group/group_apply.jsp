...
<title>集团贷</title>
	...
	<link href="<c:url value='${basePath}/web_res/css/dk/css/zzscGroup.css'/>" rel="stylesheet" />
	<script type="text/javascript" src="<c:url value='${basePath}/web_res/css/dk/js/jquery.js'/>"></script>
	<script type="text/javascript" src="<c:url value='${basePath}/web_res/css/dk/js/jquery.caroufredsel-6.0.4-packed.js'/>"></script>
	<script type="text/javascript" src="<c:url value='${basePath}/web_res/css/dk/js/focus.js'/>"></script>
	<script type="text/javascript" src="<c:url value='${basePath}/web_res/css/dk/js/jquery-1.4.4.min.js'/>"></script>
	<script type="text/javascript" src="<c:url value='${basePath}/web_res/css/dk/js/jquery.reveal.js'/>"></script>
...
/*完善借款信息样式 （增加贷款人紧急联系人、居住地址非常重要，及紧急联系人）*/
...
#layout-address .item{
	height:inherit; padding:0; text-align: left; color:#666666;
}
#layout-address .item .txt{
	padding: 0 0 5px 0;
}
#layout-address .item .adr-val{
	width:220px;padding: 0 0 3px 0; border-bottom:1px solid #cfcfcf; color:#666666;
}
._clear{
	clear:both; height:0px; font-size:0px; overflow: hidden;
}
#layout-address .item .ipt-address{
	margin-top:10px;
}
._ipt{
...
._ipt_hd{
	padding:0;color:#666666;height: inherit;
}
.m-top{
	margin-top:17px;
}
...
<body>
	...
	<div class="banner">
		<div class="banner_show" id="banner_show">
			<a class="bannger_inbox"><img src="<c:url value='${fileServerUrl }/web_res/img/banner1.jpg'/>" width="2000" height="572"/></a>
			<a class="bannger_inbox"><img src="<c:url value='${fileServerUrl }/web_res/img/banner2.jpg'/>" width="2000" height="572"/></a> 
			<a class="bannger_inbox"><img src="<c:url value='${fileServerUrl }/web_res/img/banner3.jpg'/>" width="2000" height="572"/></a>
					...
					<ul class="regist">
						<li>
			               ...
			               <c:if test="${realName== null && userThird.realName != null}">
							...
							<input type="text" name="user.realName" placeholder="真实姓名" maxlength="6" <c:if test="${userAccount.realName !=null}"> readonly="readonly" </c:if> value="${realName}" />
						</li>
						<li>
			               ...
			               <c:if test="${idNo== null && userThird.idNo != null}">
							...
							<input type="text" name="user.idNo" placeholder="身份证号" maxlength="18" <c:if test="${userAccount.idNo !=null}"> readonly="readonly" </c:if> value="${idNo}" />
						</li>
						<li>
			               ...
			               <c:if test="${mobile== null && userThird.mobile != null}">
							...
							<input type="text" name="user.mobile" placeholder="手机号码" maxlength="11" <c:if test="${userAccount.mobile !=null}"> readonly="readonly" </c:if> value="${mobile}" />
						</li>
						<li>
...
<!-- 	<div style="background: #f0f4ff"> -->
		<div style="width: 1200px; margin: 0px auto;">
			<img src="<c:url value='${fileServerUrl}/web_res/css/dk/images/ad.gif'/>" width="1200" height="421" alt="" />
		</div>
	</div>
	...
	<div id="myModal" class="reveal-modal">
		<form id="loanDetail" name="loanDetail" action="addLoanDetailGroup" method="post">
			...
			<input type="hidden" name="loanPersonDo.realName" id ="realName" value="" /> 
			<input type="hidden" name="idNo" value="" /> 
			<input type="hidden" name="mobile" value="" /> 
			<input type="hidden" name="jobDo.jobIncome" id="income" value="" />
			<div style="float: left">
				...
				<div id="layout-address">
					<a id="address" class="item"  href="javascript:;">
			            <p class="txt">选择所在地：</p>
			            <p class="menu-text adr-val" id="addressText">&nbsp;</p>
			            <input id="addressInput" type="hidden"name ="loanPersonDo.address" value="${ loanPersonDo.address}">
			            <div class="_clear"></div>
			        </a>
			        <p class="ipt-address">
			        	<input type="text" class="input-placeholder-font text-font" id="addressDetail" name ="loanPersonDo.addressDetail" value="${loanPersonDo.addressDetail}"  placeholder="居住地址，如高新南路2号碧水龙庭A区1502室">
			        </p>
				</div> 
		        
				
				<div class="container-t infos">
			        <div class="tip">紧急联系人一：所在部门负责人（部门领导）</div>
			        <div class="menu-group">
			           <a href="javascript:;" class="item no-arrow _ipt" >
				            <label>姓名：</label>
				            <input type="text" class="text-font" id="leaderName" name="loanRelationDoList[0].ralationName" value="" maxlength="6">
			           </a>
			            <a href="javascript:;" class="item no-arrow _ipt_hd">
				            <label>电话：</label>
				            <input type="tel" class=" text-font" id="leaderMobile" name="loanRelationDoList[0].mobile" value="" maxlength="11">
			           </a>           
			        </div>
			        <div class="tip m-top">紧急联系人二：直系亲属</div>
			        <div class="menu-group m-bottom">
			           <a href="javascript:;" class="item no-arrow _ipt" >
				            <label>姓名：</label>
			                <input type="text" class="text-font" id="familyName" name="loanRelationDoList[1].ralationName" value="" maxlength="6" >
			           </a> 
			            <a href="javascript:;" class="item no-arrow _ipt_hd" >
				           <label>电话：</label>
			               <input type="tel" class=" text-font" id="familyMobile" name="loanRelationDoList[1].mobile" value="" maxlength="11" >
			           </a>           
			        </div>
		       </div>
				...
				<p>
					...
					<a href="<c:url value='${fileServerUrl }/web_res/word/concat.html'/>" target="_black">《合和年在线借款协议》</a>
				</p>
				<p>
					<input type="checkbox" id="chkAgreement2"  style="width: 12px; height: 12px; margin-right: 10px;" /> 我已阅读并同意
					<a href="<c:url value='${fileServerUrl }/web_res/word/consult.html'/>" target="_black">《借款咨询服务协议》</a>
				</p>
				<p>
					<input type="checkbox" id="chkAgreement3"  style="width: 12px; height: 12px; margin-right: 10px;" /> 我已阅读并同意
					<a href="<c:url value='${fileServerUrl }/app_res/word/entrustDeal.html'/>" target="_black">《委托划款协议》</a>
				</p>
				<p>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	...
	/* 	var session ="${user}";
		...
		$("#btnSubmit").bind("click", function() {
			...
			var addressInput=$("#addressInput").val();
		 	...
		 	//var address = 
			var re_en = /^[\u4E00-\u9FA5]{2,5}$/ig;
		    var re_md = /^1{1}[3,4,5,7,8]{1}\d{9}$/gi;
			...
			if (!/^[\u4E00-\u9FA5]{2,5}$/ig.test(leaderName)) {
				alert("领导姓名只能为2到5中文字符!");
				return false;
			}
		 	if(leaderMobile=="" || leaderMobile==null){
		    	alert('请填领导手机');
	 	        return false;
		    }		    
			if (!/^1{1}[3,4,5,7,8]{1}\d{9}$/gi.test(leaderMobile)) {
				alert("领导手机号码格式有误!");
				return false;
			}
		    ...
		    if (!/^[\u4E00-\u9FA5]{2,5}$/ig.test(familyName)) {
				alert("领导姓名只能为2到5中文字符!");
				return false;
			}
		 	if(familyMobile=="" || familyMobile==null){
		    	alert('请填亲属手机');
	 	        return false;
		    }
		    if (!/^1{1}[3,4,5,7,8]{1}\d{9}$/gi.test(familyMobile)) {
				alert("亲属手机号码格式有误!");
				return false;
			}
			...
			if($("#chkAgreement3").attr("checked") != 'checked'){
				alert("请先已阅读并同意[委托划款协议]!");
				return false;
			}
			$("#loanDetail").submit();
		});
